...
import { AppButton, FlashMessage, Form, MaterialTextField, Screen } from '@reuseableComponents';
import { AuthHeader, Checkbox } from '@components';
...
export function SignUp() {
    ...
    if (!state.isCheckbox) {
      ...
      if (payload && !isEmailValid.current.isValid && !isPhoneNoValid.current.isValid) {
        ...
        Object.keys(payload).map(key => {
          if (key == 'mobile_no') {
            formData.append('mobile_no', payload.mobile_no.value)
          }
          else {
            formData.append(key, payload[key])
          }
        })
        formData.append('device_type', utility.isPlatformAndroid() ? 'android' : 'ios')
  ...
  function cbSuccess(response) {
    FlashMessage({ message: "Verification Link has been sent to your registered email", type: 'success' })
    popScreen()
  }
      ...
      <ScrollView bounces={false}>
        ...
        <Form ref={ref => (formHandler = ref)}>
          {(refCollector, onSubmitEditing, focusByRefCollectorKey) => {
            const { email, text, number, password, confirmPassword } = Form.INPUTS(refCollector, onSubmitEditing);
            return (
              <>
                <MaterialTextField
                ...
                <MaterialTextField
                  ...
                  subAnimated={{ marginTop: Platform.OS == 'android' ? Metrics.heightRatio(6) : Metrics.heightRatio(0) }}
